...
export function ResourceMap({
  ...
  // Re-center when center or zoom props change (e.g. detail page)
  useEffect(() => {
    if (!mapInstance.current || !center) return;
    mapInstance.current.setView(center, zoom ?? 14);
  }, [center, zoom]);

  // Re-render pins whenever resources change
  useEffect(() => {
  ...
  // Show user location dot when userLocation prop changes
  useEffect(() => {
    const map = mapInstance.current;
    ...
    <div className={`relative h-full w-full ${className ?? ""}`}>
      ...
      {showControls && (
        <button
          ...
          disabled={locating}
          className="absolute left-3 top-3 z-[400] inline-flex items-center gap-1.5
                     rounded-lg border border-border bg-card px-3 py-2 text-xs
                     font-semibold text-navy shadow-card transition-colors
